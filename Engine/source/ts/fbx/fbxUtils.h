...
namespace FbxUtils
{
   struct ImportOptions
   {
      ...
      //domUpAxisType  upAxis;           // Override for the Fbx <up_axis> element
      F32            unit;             // Override for the Fbx <unit> element
      eLodType       lodType;          // LOD type option
      S32            singleDetailSize; // Detail size for all meshes in the model
      String         matNamePrefix;    // Prefix to apply to Fbx material names
   ...
   // Apply the set of Fbx conditioners (suited for loading Fbx models into Torque)
   //void applyConditioners(domFbx* root);
   ...
   //void exportFbxHeader(TiXmlElement* rootNode);
   //void exportFbxMaterials(TiXmlElement* rootNode, const OptimizedPolyList& mesh, Vector<String>& matNames, const Torque::Path& FbxFile);
   //void exportFbxTriangles(TiXmlElement* meshNode, const OptimizedPolyList& mesh, const String& meshName, const Vector<String>& matNames);
   //void exportFbxMesh(TiXmlElement* rootNode, const OptimizedPolyList& mesh, const String& meshName, const Vector<String>& matNames);
   //void exportFbxScene(TiXmlElement* rootNode, const String& meshName, const Vector<String>& matNames);

   // Export an OptimizedPolyList to a simple Fbx file
   //void exportToFbx(const Torque::Path& FbxFile, const OptimizedPolyList& mesh, const String& meshName = String::EmptyString);
};
...
void CreateAndFillFbxIOSettings(KFbxSdkManager* pSdkManager);
...
bool SaveFbxScene(KFbxSdkManager* pSdkManager, KFbxDocument* pScene, const char* pFilename, int pFileFormat=-1, bool pEmbedMedia=false);
...
// Helper Classes
//
// The Fbx DOM uses a different class for each XML element, and there is very
...
/// Convert from the Fbx transform types to a Torque MatrixF
template<class T> inline MatrixF vecToMatrixF(const domListOfFloats& vec) { return MatrixF(true); }

/// Fbx <translate>: [x_translate, y_translate, z_translate]
...
/// Fbx <scale>: [x_scale, y_scale, z_scale]
...
/// Fbx <rotate>: [rotation_axis, angle_in_degrees]
...
/// Fbx <matrix>: same form as TGE (woohoo!)
...
/// Fbx <skew>: [angle_in_degrees, rotation_axis, translation_axis]
...
//-----------------------------------------------------------------------------
// Fbx <source>s are extremely flexible, and thus difficult to access in a nice
// way. This class attempts to provide a clean interface to convert Fbx source
// data to the appropriate Torque data structure without losing any of the flexibility
// of the underlying Fbx DOM.
//
...
class _SourceReader
{
   const domSource* source;      // the wrapped Fbx source
...
//-----------------------------------------------------------------------------
// Fbx geometric primitives: Use the BasePrimitive class to access the
...
public:
   ...
   static bool isPrimitive(const daeElement* element)
   {
      switch (element->getElementType()) {
         case Fbx_TYPE::TRIANGLES:          case Fbx_TYPE::POLYLIST:
         case Fbx_TYPE::POLYGONS:           case Fbx_TYPE::TRIFANS:
         case Fbx_TYPE::TRISTRIPS:          case Fbx_TYPE::CAPSULE:
         case Fbx_TYPE::CYLINDER:           case Fbx_TYPE::LINES:
         case Fbx_TYPE::LINESTRIPS:         case Fbx_TYPE::PLANE:
         case Fbx_TYPE::SPLINE:             case Fbx_TYPE::SPHERE:
         case Fbx_TYPE::TAPERED_CAPSULE:    case Fbx_TYPE::TAPERED_CYLINDER:
            return true;
      }
      return false;
   }
   ...
   {
      switch (element->getElementType()) {
         case Fbx_TYPE::TRIANGLES:
         case Fbx_TYPE::TRISTRIPS:
         case Fbx_TYPE::TRIFANS:
         case Fbx_TYPE::POLYLIST:
         case Fbx_TYPE::POLYGONS:
            return true;
      }
      return false;
   }

   /// Construct a child class based on the type of Fbx element
   static BasePrimitive* get(const daeElement* element);

   /// Methods to be implemented for each supported Fbx geometric element
...
/// Template child class for supported Fbx primitive elements
template<class T> class FbxPrimitive : public BasePrimitive
{
   T* primitive;
   domListOfUInts *pTriangleData;
   S32 stride;
public:
   FbxPrimitive(const daeElement* e) : pTriangleData(0)
   {
...
//-----------------------------------------------------------------------------
// <triangles>
template<> inline const domListOfUInts *FbxPrimitive<domTriangles>::getTriangleData()
{
...
//-----------------------------------------------------------------------------
// <tristrips>
template<> inline const domListOfUInts *FbxPrimitive<domTristrips>::getTriangleData()
{
   if (!pTriangleData)
   {
...
//-----------------------------------------------------------------------------
// <trifans>
template<> inline const domListOfUInts *FbxPrimitive<domTrifans>::getTriangleData()
{
   if (!pTriangleData)
   {
      // Convert strips to triangles
...
//-----------------------------------------------------------------------------
// <polygons>
template<> inline const domListOfUInts *FbxPrimitive<domPolygons>::getTriangleData()
{
   if (!pTriangleData)
   {
...
//-----------------------------------------------------------------------------
// <polylist>
template<> inline const domListOfUInts *FbxPrimitive<domPolylist>::getTriangleData()
{
   if (!pTriangleData)
   {
      // Convert polygons to triangles
      ...
      // has been seen with certain models exported using FbxMax)
...
struct AnimData
{
   ...
   /// Get the animation channels for the Fbx element (if any)
...
//-----------------------------------------------------------------------------
// Fbx allows any element with an SID or ID attribute to be the target of
...
{
   const daeElement* element;    ///< The Fbx element (can be NULL)
