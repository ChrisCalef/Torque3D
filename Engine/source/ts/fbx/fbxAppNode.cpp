    ...
    //  invertMeshes(false)
{
	...
	transform.set(EulerF(mDegToRad(rVec[0]),mDegToRad(rVec[1]),mDegToRad(rVec[2])));
	...
	if (gBindPose)
	{
		for (U32 i=0;i<gBindPose->GetCount();i++)
		{
			...
			if (gBindPose->GetNode(i)==node)
			{
				...
				//	mRadToDeg(kEuler2.x),mRadToDeg(kEuler2.y),mRadToDeg(kEuler2.z));
				invBindTransform.identity();
				//invBindTransform.mul(preRotMat);//should this be before or after the bindTransform?
				invBindTransform.mul(preRotMat);
...
*/
	...
	//Con::printf("Node trying to use scale factor: %f",mScaleFactor);
	if (parent)
	{
	...
	//Con::printf("%s  tVec:  %f %f %f,  rVec:  %f %f %f    pos %f %f %f",
	//	node->GetName(),tVec[0],tVec[1],tVec[2],rVec[0],rVec[1],rVec[2],pos2.x,pos2.y,pos2.z);
