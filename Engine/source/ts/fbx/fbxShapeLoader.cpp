...
bool FbxShapeLoader::LoadScene(const char* pFilename)
{
    ...
    if( !lImportStatus )
    {
	 ...
	 if (lImporter->IsFBX())
	 {
		 ...
		 //TEMP:  now save every fbx file as an ascii version of itself, so I can see what the hell is going on.
		 //Later, add this functionality below under SaveScene, so I don't have to do it in flexbody anymore.

		 //String outputExt(".ascii.fbx");
		 //String inputName = pFilename;
		 //String outputName = inputName + outputExt;

		 //SaveFbxScene(mSdkManager,mScene,outputName.c_str(),1,false);
...
void FbxShapeLoader::enumerateScene()
{
	...
	for (U32 i=0;i<mScene->GetPoseCount();i++)
	{
		...
		else if (kPose->IsRestPose())
		{
			Con::printf("Found a Rest Pose %s",kPose->GetName());
		} 
	}
...
void FbxShapeLoader::examineScene(KFbxImporter* lImporter)
{
	...
	for (int i=0;i<mScene->GetMemberCount();i++)
	{
		Con::printf("Member %d: %s",i,mScene->GetMember(i)->GetName());
	}
	...
	if (appNode->nodeTransforms.size()>frame)
	{
		...
		if (appNode->mParentIndex >= 0)
		{
			...
			if (parentNode)
			{
				...
				//Point3F temp(-pos.x,pos.z,pos.y);//HMM, odd, seems not to be necessary??
				//pos = temp;
			}
		} else {		
		...
		m1.setPosition(Point3F(mPos.x/mFbxScale,mPos.y/mFbxScale,mPos.z/mFbxScale));//Also not having any effect??
	}
	...
	for (U32 i=0;i<mScene->GetMemberCount();i++)
	{
		...
		else if (!strcmp(member->GetTypeName().Buffer(),"LimbNode"))
		{
			...
			if (kNode)
			{
				...
				if (numSeqs>0)
					appSeq = dynamic_cast<FbxAppSequence*>(appSequences.first());//.last()?
				else
					continue;
				...
				while (prop.IsValid())
				{
					if (!strcmp(prop.GetLabel().Buffer(),"Lcl Translation"))
					{
						...
						if (curveNode)
						{
							...
							//Con::printf("Translation node: %f %f %f",kCurve_X,kCurve_Y,kCurve_Z);
							...
							if (kFCurve_X  &&  kFCurve_Y  &&  kFCurve_Z)
							{
								...
								{//(Have to assume they all have same framecount.)
									...
									//Con::printf("GOT A KFCURVE!  %f %f %f",lKeyX,lKeyY,lKeyZ);
