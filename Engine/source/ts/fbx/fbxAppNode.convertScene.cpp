    ...
    //  invertMeshes(false)
{
	...
	Point3F pos,pos2;
	MatrixF defTransform,bindTransform,invBindTransform,transform,parentM;
	EulerAngles ea;
	HMatrix	hMatrix;
	MatrixF m1;
	...
	//Con::printf("\nLclTranslation: %f %f %f",lclTrans[0],lclTrans[1],lclTrans[2]);
	//
	fbxDouble3 lclRot;
	lclRot = node->LclRotation.Get();
	//Con::printf("LclRotation: %f %f %f",lclRot[0],lclRot[1],lclRot[2]);
	
	//fbxDouble3 lclScale;
	//lclScale = node->LclScaling.Get();
	//Con::printf("LclScaling: %f %f %f",lclScale[0],lclScale[1],lclScale[2]);

	//pos.set(-tVec[0],tVec[2],tVec[1]);//OOPS, this would be twice, tsShapeLoader already does this.
	defTransform.set(EulerF(mDegToRad(rVec[0]),mDegToRad(rVec[1]),mDegToRad(rVec[2])));
	Con::printf("LclRotation: %f %f %f    rVec  %f %f %f",lclRot[0],lclRot[1],lclRot[2],rVec[0],rVec[1],rVec[2]);
	...
	if (gBindPose)
	{
		for (U32 i=0;i<gBindPose->GetCount();i++)
		{
			...
			if (gBindPose->GetNode(i)==node)
			{
				...
				//bindTransform.setPosition(Point3F(Row3[0]*sc,Row3[1]*sc,Row3[2]*sc));
				mBindTransform = newBindTrans;
				//kRow.set(Row0[0],Row1[0],Row2[0],Row3[0]); bindTransform.setRow(0,kRow);//Or, not.
				//kRow.set(Row0[1],Row1[1],Row2[1],Row3[1]); bindTransform.setRow(1,kRow);//Appeared that FBX was column major,
				//kRow.set(Row0[2],Row1[2],Row2[2],Row3[2]); bindTransform.setRow(2,kRow);//but changing it wrecked even worse.
				//kRow.set(Row0[3],Row1[3],Row2[3],Row3[3]); bindTransform.setRow(3,kRow);
				
				//NEGATORY!!!  Bind transforms are GLOBAL, meaning parent is already included.
				//if (parent)
				//{  
					//MatrixF parentBindTrans = parent->mBindTransform;
					//bindTransform.mul(parentBindTrans);
				//}
				QuatF bindQuat(bindTransform);
				//Point3F bindPos = bindTransform.getPosition();
				Con::printf("We found our bind node!!!  Euler %f %f %f ",
					mRadToDeg(kEuler.x),mRadToDeg(kEuler.y),mRadToDeg(kEuler.z));
				invBindTransform.identity();
				//invBindTransform.mul(preRotMat);//should this be before or after the bindTransform?
				invBindTransform.mul(newBindTrans.inverse());
				invBindTransform.mul(preRotMat.inverse());
				mBindTransform = newBindTrans; //Having been inverted now...
			}
		}
	}
	//transform
	pos.set(tVec[0],tVec[1],tVec[2]);
	//pos.set(-tVec[0],tVec[2],tVec[1]); //COORDSHIFT
	pos *= sc;//If it's truly in centimeters, then that is one/100th of a meter.
	
	//transform = defTransform;
	//transform.mul(invBindTransform);
	transform = invBindTransform;
	//transform.mul(defTransform);//NO!  It is either this, a local rotation, OR the bind matrix which INCLUDES local rot.  Not both!
	tempNodeTransform = transform;
	transform.mulP(pos,&pos2);
	//defaultNodeTransform = transform;
	//transform = defTransform;
	Point3F parentPos;
	if (parent)
	{
		//parentM = parent->mBindTransform;
		//parentPos = parentM.getPosition();
		Con::printf("node %s has parent %s  pos %f %f %f",node->GetName(),parent->mName,parentPos.x*100.0,parentPos.y*100.0,parentPos.z*100.0);
		parentM = parent->tempNodeTransform;//bind transform plus local transform, of parent
		transform.mul(parentM.inverse());//take this out to get local transform of this node
		//tempNodeTransform = transform;
		//transform.mulP(pos,&pos2);//and this should already be local transform, so don't add or subtract anything
		KFbxVector4 parentPos;
		Point3F pPos,pPos2;
		parent->mNode->GetDefaultT(parentPos);
		//pPos = Point3F(parentPos[0],parentPos[2],parentPos[1]); //COORDSHIFT
		pPos = Point3F(parentPos[0],parentPos[1],parentPos[2]);
		pPos *= sc;
		//parent->mBindTransform.mulP(pPos,&pPos2);
		pos2 += pPos;
		//mFinalPos = pos2;
	} else {
		Con::printf("node %s has no parent:  %s",node->GetName(),node->GetParent()->GetName());
		KFbxVector4 parentRotVec,parentPosVec;
		node->GetParent()->GetDefaultR(parentRotVec);
		node->GetParent()->GetDefaultT(parentPosVec);
		//parentM.set(EulerF(mDegToRad(parentRotVec[0]),mDegToRad(parentRotVec[1]),mDegToRad(parentRotVec[2])));
		parentPos.set(Point3F(parentPosVec[0],parentPosVec[1],parentPosVec[2]));
		//parentPos.set(Point3F(-parentPosVec[0],parentPosVec[2],parentPosVec[1])); //COORDSHIFT
		//transform.mul(parentM.inverse());
		//transform.mulP(pos,&pos2);
		//parentPos *= 1.0/100.0;
		pos2 += parentPos;
		//tempNodeTransform = transform;
	...
	Con::printf("%s  parent %s tVec:  %f %f %f,  rVec:  %f %f %f    pos %f %f %f\n",
		node->GetName(),node->GetParent()->GetName(),tVec[0],tVec[1],tVec[2],rVec[0],rVec[1],rVec[2],pos2.x*100.0,pos2.y*100.0,pos2.z*100.0);
	
}


/*
KFbxMesh* mesh = (KFbxMesh*) i_Node->GetNodeAttribute();
...
for( int s = 0; s < numSkins; ++s)
{
	...
	for(int c = 0 ; c < numClusters; ++c)
	{
		...
		if ( boneNode == NULL )
		{
			sprintf(message,” Cluster [%d] No bone attached\n”,c);PrintMessage(message);
			continue;
		}
...
void FbxAppNode::buildMesh(FbxAppMesh *appMesh)
{
	...
	if (kNode)
	{
		...
		//Con::printf("Found my mesh node: ptVec %f %f %f, prVec %f %f %f %f",ptVec[0],ptVec[1],ptVec[2],prVec[0],prVec[1],prVec[2],prVec[3]);
		appMesh->mOffset.set(tVec[0]*sc,tVec[1]*sc,tVec[2]*sc);
		//appMesh->mOffset.set(-tVec[0]*sc,tVec[2]*sc,tVec[1]*sc); //COORDSHIFT
		...
		*/
		...
		//appMesh->mOrientation.set(EulerF(mDegToRad(preRotSrc[0]),mDegToRad(-preRotSrc[2]),mDegToRad(-preRotSrc[1])));
		...
		rotOffsetMat.set(EulerF(mDegToRad(rotOffsetSrc[0]),mDegToRad(rotOffsetSrc[1]),mDegToRad(rotOffsetSrc[2])));
		rotPivotMat.set(EulerF(mDegToRad(preRotSrc[0]),mDegToRad(preRotSrc[1]),mDegToRad(preRotSrc[2])));
		//geomRotMat.set(EulerF(mDegToRad(geomRotSrc[0]),mDegToRad(-geomRotSrc[2]),mDegToRad(-geomRotSrc[1])));  //COORDSHIFT
		//postRotMat.set(EulerF(mDegToRad(postRotSrc[0]),mDegToRad(-postRotSrc[2]),mDegToRad(-postRotSrc[1]))); //COORDSHIFT
		//preRotMat.set(EulerF(mDegToRad(preRotSrc[0]),mDegToRad(-preRotSrc[2]),mDegToRad(-preRotSrc[1])));     //COORDSHIFT
		//rotOffsetMat.set(EulerF(mDegToRad(rotOffsetSrc[0]),mDegToRad(-rotOffsetSrc[2]),mDegToRad(-rotOffsetSrc[1])));  //COORDSHIFT
		//rotPivotMat.set(EulerF(mDegToRad(preRotSrc[0]),mDegToRad(-preRotSrc[2]),mDegToRad(-preRotSrc[1])));   //COORDSHIFT
		
		finalMult.identity();
		finalMult.mul(rotOffsetMat);
		finalMult.mul(preRotMat);
		finalMult.mul(postRotMat);
		finalMult.mul(geomRotMat);//????
		//finalMult.mul(rotPivotMat);//????


		appMesh->mOrientation = finalMult;
		//World = ParentWorld * T * Roff * Rp * Rpre * R * Rpost * Rp-1 * Soff * Sp * S * Sp-1 
	}
	...
	{			
		...
		Point3F pos(thisPoint[0]*sc,thisPoint[1]*sc,thisPoint[2]*sc);
		//Point3F pos(-thisPoint[0]*sc,thisPoint[2]*sc,thisPoint[1]*sc); //COORDSHIFT
		...
		appMesh->normals.push_back(Point3F(0,0,1));// here, then assign values later.
	...
	{			
		...
		{//HERE: figure these out above, with uvs.
			...
			Point3F normal(norm[0],norm[1],norm[2]);
			//Point3F normal(-norm[0],norm[2],norm[1]); //COORDSHIFT
