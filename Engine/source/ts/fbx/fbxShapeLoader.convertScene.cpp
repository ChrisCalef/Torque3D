...
KFbxPose *gBindPose = NULL;//FIX: this should be somebody's property, but whose?
...
TSShape* loadFbxShape(const Torque::Path &path)
{
   ...
   TSShape* tss = 0;
	...
	FbxShapeLoader loader;//HERE: how about deleting this when we're done??  Oh, it's local so should get removed at end of function.
...
FbxShapeLoader::FbxShapeLoader()
{
	...
	mFbxClusters = 0;
	mFbxActors = 0;
	mFbxScale = 100.0;//Default meters to centimeters - expose this on fbx import GUI when we get there.
	mFbxRootNode = NULL;
...
bool FbxShapeLoader::LoadScene(const char* pFilename)
{
	...
	//if (!strcmp(member->GetName(),"T")) {
	//	 Con::printf("      FOUND T!!!!!!!!!!!!");
	//} else if (!strcmp(member->GetName(),"R")) {
	//	 Con::printf("      FOUND R!!!!!!!!!!!!");
	//}
...
void FbxShapeLoader::enumerateScene()
{
	...
	//COORDSHIFT
	//Well, nice try anyway.  Trying again without.
	...
	for (U32 i=0;i<mScene->GetPoseCount();i++)
	{
		...
		if (kPose->IsBindPose())
		{
			...
			if (gBindPose)
			{
				...
				{
					Point4F kRow;
					if (1)//(gBindPose->GetNode(i)==node)
					{
						KFbxMatrix matrix = gBindPose->GetMatrix(i);	
						MatrixF bindTransform,invBindTransform;
						...
						invBindTransform = bindTransform.inverse();
						EulerF kEuler = invBindTransform.toEuler();
						Point3F pos = invBindTransform.getPosition();
						QuatF bindQuat(invBindTransform);
						Con::printf("bind node!!!  Euler %f %f %f  node %s  pos %f %f %f",
							mRadToDeg(kEuler.x),mRadToDeg(kEuler.y),mRadToDeg(kEuler.z),gBindPose->GetNode(i)->GetName(),
							pos.x,pos.y,pos.z);
	...
	//Con::printf("Pose %s is bind pose: %d,    count %d",kPose->GetName(),isBind,kPose->GetCount());
	//for (U32 i=0;i<kPose->GetCount();i++)
	//{
	//	KFbxPoseInfo *poseInfo = kPose->GetItem(0);
	//	KFbxMatrix matrix = poseInfo->mMatrix;
	//	
	//}


	bool kHasCurveNodes = false;
	bool kHasAnimLayer = false;

	for (U32 i = 0; i < mScene->GetMemberCount(); i++)
	{
		KFbxObject *member = mScene->GetMember(i);
		KFbxAnimCurveNode *kAnimCurveNode = NULL;
		kAnimCurveNode = dynamic_cast<KFbxAnimCurveNode *>(member);
		if (kAnimCurveNode)
		{
			kHasCurveNodes = true;
		}
	...
	for (U32 i = 0; i < mScene->GetMemberCount(); i++)
	{
		...
		if ((kAnimStack)&&(kHasAnimLayer)&&(kHasCurveNodes))
		{//Hmm, above doesn't work, still crashes. 
...
void FbxShapeLoader::examineScene(KFbxImporter* lImporter)
{
	...
	{
		//int upVecSign=1;
		//Con::printf("Axis system up vector: %d",SceneAxisSystem.GetUpVector(upVecSign));

		//Con::printf("Axis system coord system: %d",SceneAxisSystem.GetCoorSystem());
		//
		...
		Con::printf("system unit scale factor(cm): %f",SceneSystemUnit.GetScaleFactor());
...
void FbxShapeLoader::examineMember(KFbxObject *member)
{
	...
	if (!strcmp(member->GetTypeName().Buffer(),"Mesh")) 
	{
		...
		//Con::printf("Found a Rotation: %f %f %f",0,0,0);
		//KFbxObject *dstObj = member->GetDstObject();
		//if (dstObj) {
		//	Con::printf("Got a dst object: %s",dstObj->GetName()); 

		//	KFbxObject *subDstObj = dstObj->GetDstObject();
		//	if (subDstObj) 
		//		Con::printf("Got a sub dst object: %s",subDstObj->GetName()); 
		//	KFbxObject *subSrcObj = dstObj->GetSrcObject();
		//	if (subSrcObj) 
		//		Con::printf("Got a sub src object: %s",subSrcObj->GetName()); 
		//}
		//KFbxObject *srcObj = member->GetSrcObject();
		//if (srcObj) Con::printf("Got a src object: %s",srcObj->GetName());
	...
	if (appNode->nodeTransforms.size()>frame)
	{
		...
		if (appNode->mParentIndex >= 0)
		{
			...
			if (parentNode)
			{
				...
				//Point3F temp(-pos.x,pos.z,pos.y); //COORDSHIFT
				//pos = temp;
			}
		} else {		
		...
		//m1.setPosition(Point3F(-mPos.x/mFbxScale,mPos.z/mFbxScale,mPos.y/mFbxScale)); //COORDSHIFT
		m1.setPosition(Point3F(mPos.x/mFbxScale,mPos.y/mFbxScale,mPos.z/mFbxScale));
	}
...
{//This is for the root node of the skeleton, *not* scene root node or any other node.
	...
	KFbxVector4 pos,rot;
	node->GetDefaultT(pos);
	node->GetDefaultR(rot);
	Con::printf("  skeleton root node: %s   type  %s   childcount %d  defaultT %f %f %f  defaultR %f %f %f",node->GetName(),
				node->GetTypeName().Buffer(),node->GetChildCount(),pos[0],pos[1],pos[2],rot[0],rot[1],rot[2]);
...
void FbxShapeLoader::postEnumerateScene()
{
	...
	//Now, add clusters and sequences, AFTER we have generated our AppNodes.
	...
	for (U32 i=0;i<mScene->GetMemberCount();i++)
	{
		...
		if (!strcmp(member->GetTypeName().Buffer(),"Cluster")) 
		{
			...
			if (kCluster)
			{
				...
				kCluster->GetTransformMatrix(lMeshGlobal);
				KFbxVector4 meshR,meshT;
				meshR = lMeshGlobal.GetR();
				meshT = lMeshGlobal.GetT();
				Con::printf("cluster %d mesh R %f %f %f, T %f %f %f",i,meshR[0],meshR[1],meshR[2],meshT[0],meshT[1],meshT[2]);
		...
		else if (!strcmp(member->GetTypeName().Buffer(),"LimbNode"))
		{
			...
			if (kNode)
			{
				...
				mSceneEvaluator = mScene->GetEvaluator();
				KTime myTime; // The time for each key in the animation curve(s)
				myTime.SetSecondDouble(0.0);
				MatrixF evalMatrix;
				Point4F kRow;

				//KFbxXMatrix& GlobalMatrix = mSceneEvaluator->GetNodeGlobalTransform(kNode, myTime);
				//KFbxVector4 Row0 = GlobalMatrix.GetRow(0);
				//KFbxVector4 Row1 = GlobalMatrix.GetRow(1);
				//KFbxVector4 Row2 = GlobalMatrix.GetRow(2);
				//KFbxVector4 Row3 = GlobalMatrix.GetRow(3);
				KFbxXMatrix& LocalMatrix = mSceneEvaluator->GetNodeLocalTransform(kNode, myTime);
				KFbxVector4 Row0 = LocalMatrix.GetRow(0); 
				KFbxVector4 Row1 = LocalMatrix.GetRow(1);
				KFbxVector4 Row2 = LocalMatrix.GetRow(2);
				KFbxVector4 Row3 = LocalMatrix.GetRow(3);
				kRow.set(Row0[0],Row0[1],Row0[2],Row0[3]); evalMatrix.setRow(0,kRow);
				kRow.set(Row0[0],Row0[1],Row0[2],Row0[3]);  evalMatrix.setRow(0,kRow);
				kRow.set(Row0[0],Row0[1],Row0[2],Row0[3]); evalMatrix.setRow(2,kRow);
				kRow.set(Row0[0],Row0[1],Row0[2],Row0[3]);  evalMatrix.setRow(3,kRow);
				Con::printf("Row0: %f  %f  %f  %f",Row0[0],Row0[1],Row0[2],Row0[3]); 
				Con::printf("Row1: %f  %f  %f  %f",Row1[0],Row1[1],Row1[2],Row1[3]);
				Con::printf("Row2: %f  %f  %f  %f",Row2[0],Row2[1],Row2[2],Row2[3]);
				Con::printf("Row3: %f  %f  %f  %f",Row3[0],Row3[1],Row3[2],Row3[3]);
				...
				while (prop.IsValid())
				{
					if (!strcmp(prop.GetLabel().Buffer(),"Lcl Translation"))
					{
						...
						if (curveNode)
						{
							...
							if (kFCurve_X  &&  kFCurve_Y  &&  kFCurve_Z)
							{
								...
								Con::printf("GOT A KFCURVE!  X count %d   Y count %d   Z count %d  node %s",
									kFCurve_X->KeyGetCount(),kFCurve_Y->KeyGetCount(),kFCurve_Z->KeyGetCount(),appNode->getName());
								...
								{//(Have to assume they all have same framecount.)
									...
									//Point3F pos(-lKeyX,lKeyZ,lKeyY); //COORDSHIFT
									Point3F pos(lKeyX,lKeyY,lKeyZ);
									pos *= 1.0/mFbxScale;
					...
					{
						Con::printf("found a local Rotation curve node");
						KFbxAnimCurveNode *curveNode = NULL;
						KFCurve *kFCurve_X,*kFCurve_Y,*kFCurve_Z;
						KFbxAnimCurve *kCurve_X,*kCurve_Y,*kCurve_Z;
						curveNode = prop.GetCurveNode();
						if (curveNode)
						{
							...
							if (kFCurve_X  &&  kFCurve_Y  &&  kFCurve_Z)
							{
								...
								{//(Have to assume they all have same framecount.)
									...
									EulerF eul(lKeyX * (M_PI/180.0),lKeyY* (M_PI/180.0),lKeyZ * (M_PI/180.0));//FIX mDeg2Rad
									//EulerF eul(lKeyX * (M_PI/180.0),-lKeyZ * (M_PI/180.0),-lKeyY * (M_PI/180.0)); //COORDSHIFT?
									//EulerF eul(-lKeyX * (M_PI/180.0),lKeyZ * (M_PI/180.0),-lKeyY * (M_PI/180.0));
									//EulerF eul(lKeyX * (M_PI/180.0),lKeyZ * (M_PI/180.0),-lKeyY * (M_PI/180.0));
									//EulerF eul(-lKeyX * (M_PI/180.0),-lKeyZ * (M_PI/180.0),-lKeyY * (M_PI/180.0));
									...
									//appNode->nodeRotates.last() = evalMatrix;
									//Con::printf("    key %d:  value %f  %f  %f",i,lKeyX,lKeyY,lKeyZ);
								}
							}
						}
					}					
					else if (!strcmp(prop.GetLabel().Buffer(),"Lcl Scaling")) 
					{
						Con::printf("found a local Scaling curve node");
					}
NEXT:
					prop = member->GetNextProperty(prop);
				}				
